...
class db(object):
    ...
    def insertTrnutniZadatak(self,zadatak):
        try:
            query = "INSERT INTO trenutniZadatak (komponenta,bin,zadato,izmereno,zakljucen) VALUES(?,?,?,?,?)"
            args = (zadatak.komponenta,zadatak.bin,zadatak.zadato,zadatak.izmereno,zadatak.zakljucen)
            self.cur.execute(query, args)
            #self.cursor.executemany(query, books)
            self.con.commit()
            return True
    ...
    def updateIzmereno(self, id, izmereno,zakljucen):
        try:  
            query ="UPDATE trenutniZadatak set izmereno = "+str(izmereno)+" where id="+str(id)
            self.cur.execute(query)
            self.con.commit()
            query ="UPDATE trenutniZadatak set zakljucen = "+str(zakljucen)+" where id="+str(id)
            self.cur.execute(query)
            self.con.commit()
            return True
        except:
